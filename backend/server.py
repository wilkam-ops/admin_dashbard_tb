...
from models import (
    User, UserCreate, UserLogin, UserInDB, Token, UserUpdate,
    Course, CourseCreate, CourseUpdate,
    TeeTime, TeeTimeCreate, TeeTimeUpdate,
    Booking, BookingCreate, BookingStatus,
    Competition, CompetitionCreate, CompetitionUpdate, CompetitionStatus,
...
async def update_course(
    ...
    update_data = {k: v for k, v in course_data.dict(exclude_unset=True).items()}
    if update_data:
...
@api_router.get("/tee-times/{tee_time_id}", response_model=TeeTime)
async def get_tee_time(tee_time_id: str):
    tee_time = await db.tee_times.find_one({"id": tee_time_id})
    if not tee_time:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Tee time not found"
        )
    return TeeTime(**tee_time)

@api_router.put("/tee-times/{tee_time_id}", response_model=TeeTime)
async def update_tee_time(
    tee_time_id: str,
    tee_time_data: TeeTimeUpdate,
    _: str = Depends(get_current_admin)
):
    tee_time = await db.tee_times.find_one({"id": tee_time_id})
    if not tee_time:
    ...
    update_data = {k: v for k, v in tee_time_data.dict(exclude_unset=True).items()}
    if update_data:
    ...
    updated_tee_time = await db.tee_times.find_one({"id": tee_time_id})
    return TeeTime(**updated_tee_time)

@api_router.delete("/tee-times/{tee_time_id}")
async def delete_tee_time(
    tee_time_id: str,
    _: str = Depends(get_current_admin)
):
    result = await db.tee_times.delete_one({"id": tee_time_id})
    if result.deleted_count == 0:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Tee time not found"
...
async def update_competition(
    ...
    update_data = {k: v for k, v in competition_data.dict(exclude_unset=True).items()}
    if update_data:
    ...
    return Competition(**updated_competition)
...
async def update_user(
    ...
    return User(
        ...
        handicapIndex=updated_user.get("handicapIndex"),
